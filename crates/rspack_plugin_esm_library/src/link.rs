...
impl EsmLibraryPlugin {
  fn ensure_raw_import_binding(
    chunk_link: &mut ChunkLinkContext,
    raw_import_source: RawImportSource,
    import_binding: ExternalImportBinding,
    requested_local: &Atom,
  ) -> Atom {
    ...
    match import_binding {
      ExternalImportBinding::Default => {
        if let Some(existing_local) = &import_spec.default_import {
          existing_local.clone()
        } else {
          import_spec.default_import = Some(requested_local.clone());
          chunk_link.used_names.insert(requested_local.clone());
          requested_local.clone()
        }
      }
      ExternalImportBinding::Named(imported_name) => {
        let has_requested_alias = import_spec
          .atoms
          .get(&imported_name)
          .is_some_and(|local| local == requested_local)
          || import_spec
            .named_imports
            .iter()
            .any(|(imported, local)| imported == &imported_name && local == requested_local);

        if !has_requested_alias {
          if import_spec.atoms.contains_key(&imported_name) {
            import_spec
              .named_imports
              .push((imported_name, requested_local.clone()));
          } else {
            import_spec
              .atoms
              .insert(imported_name, requested_local.clone());
          }
        }
        chunk_link.used_names.insert(requested_local.clone());
        requested_local.clone()
      ...
      ExternalImportBinding::Namespace => {
        if let Some(existing_local) = &import_spec.ns_import {
          existing_local.clone()
        } else {
          import_spec.ns_import = Some(requested_local.clone());
          chunk_link.used_names.insert(requested_local.clone());
          requested_local.clone()
        }
      }
  ...
  fn ensure_existing_named_import_alias(
    chunk_link: &mut ChunkLinkContext,
    imported_name: &Atom,
    local_name: &Atom,
  ) -> bool {
    for import_spec in chunk_link.raw_import_stmts.values_mut() {
      if import_spec
        .atoms
        .get(imported_name)
        ...
        || import_spec
          .named_imports
          .iter()
          .any(|(imported, local)| imported == imported_name && local == local_name)
      {
        chunk_link.used_names.insert(local_name.clone());
        return true;
      }

      if let Some(existing_local) = import_spec.atoms.get_mut(imported_name) {
        *existing_local = local_name.clone();
        import_spec
          .named_imports
          .retain(|(imported, local)| imported != imported_name || local != local_name);
        chunk_link.used_names.insert(local_name.clone());
        return true;
      }
    }
  ...
  fn import_spec_uses_local(import_spec: &ImportSpec, local_name: &Atom) -> bool {
    import_spec
      .atoms
      .values()
      .any(|existing_local| existing_local == local_name)
      || import_spec
        .named_imports
        .iter()
        .any(|(_, existing_local)| existing_local == local_name)
      || import_spec.default_import.as_ref() == Some(local_name)
      || import_spec.ns_import.as_ref() == Some(local_name)
      || import_spec.ns_imports.contains(local_name)
  }
  ...
  fn get_external_import_source_and_binding(
    ...
    let lookup_name = info
      .raw_export_map
      .as_ref()
      .and_then(|raw_export_map| {
        raw_export_map
          .iter()
          .find_map(|(export_name, symbol)| (symbol == local_name.as_str()).then_some(export_name))
      })
    ...
    if let Some(import_map) = info.import_map.as_ref() {
      for ((source, attr), imported_atoms) in import_map {
        ...
        if imported_atoms
          .namespace
          .as_ref()
          .is_some_and(|namespace| namespace == lookup_name)
          || imported_atoms.namespace_aliases.contains(lookup_name)
          || info.namespace_object_name.as_ref() == Some(lookup_name)
        {
        ...
        for imported_name in &imported_atoms.specifiers {
          ...
          if internal_name != lookup_name && imported_name != lookup_name {
            continue;
          }
        ...
        for (imported_name, imported_local_name) in &imported_atoms.specifier_aliases {
          let internal_name = info
            .get_internal_name(imported_local_name)
            .unwrap_or(imported_local_name);
          if internal_name != lookup_name && imported_local_name != lookup_name {
            continue;
          }
    ...
    if info.namespace_object_name.as_ref() == Some(lookup_name) {
  ...
  pub(crate) async fn link(
    ...
    let (escaped_name_entries, escaped_identifier_entries) = concate_modules_map
      ...
      .map(|info| {
          ...
          ModuleInfo::Concatenated(info) => {
            ...
            let imported_names = import_map.map_or(0, |map| {
              map
                .values()
                .map(|imported| {
                  imported.specifiers.len()
                    + usize::from(imported.namespace.is_some())
                    + imported.namespace_aliases.len()
                })
                .sum::<usize>()
            });
            (
        ...
        let (readable_identifier, splitted_readable_identifier) =
          get_cached_readable_identifier_with_parts(
            &info.id(),
            module_graph,
          ...
          ModuleInfo::Concatenated(info) => {
            ...
            if let Some(import_map) = &info.import_map {
              for ((source, _), imported_atoms) in import_map.iter() {
                ...
                for atom in imported_atoms.specifier_aliases.values() {
                  escaped_names
                    .entry(atom.clone())
                    .or_insert_with(|| escape_name_atom_ref(atom));
                }
                ...
                for ns_import in &imported_atoms.namespace_aliases {
                  escaped_names
                    .entry(ns_import.clone())
                    .or_insert_with(|| escape_name_atom_ref(ns_import));
                }
    ...
    for (ukey, mut needed_namespace_objects) in needed_namespace_objects_by_ukey {
      ...
      loop {
        ...
        for module_info_id in needed_namespace_objects.clone().iter() {
          ...
          let namespace_export_symbol = module_info.namespace_export_symbol.clone();
          let namespace_export_entries = module_info
            .export_map
            .iter()
            .flat_map(|map| map.iter())
            .chain(module_info.raw_export_map.iter().flat_map(|map| map.iter()))
            .map(|(export_name, symbol)| (export_name.clone(), symbol.clone()))
            .collect::<Vec<_>>();
          let namespace_internal_names = module_info.internal_names.clone();
          let is_external_module = box_module.as_external_module().is_some();

          if is_external_module && namespace_export_symbol.is_some() {
            continue;
          }

          let mut ns_obj = Vec::new();
          let mut ns_obj_exports = FxHashSet::default();
            ...
            .values()
          {
            ...
            if namespace_export_symbol.is_none()
              && matches!(export_info.get_used(None), UsageState::Unused)
            {
              // skip useless
              continue;
            }
            ...
            if let Some(UsedNameItem::Str(used_name)) = export_info.get_used_name(None, None) {
                  ...
                  && matches!(target_info, Some(ModuleInfo::Concatenated(_)))
                {
                      ...
                      || ext.resolve_external_type() == "module")
                  {
                    ...
                    let requested_local =
                      if !matches!(
                        import_binding,
                        ExternalImportBinding::Namespace | ExternalImportBinding::Named(_)
                    ...
                    let local_name = Self::ensure_raw_import_binding(
                      chunk_link,
                      raw_import_source,
                      import_binding,
                      &requested_local,
                    ...
                    if local_name != symbol_binding.symbol
                      && let Some(ModuleInfo::Concatenated(target_info)) =
                        concate_modules_map.get_mut(&symbol_binding.module)
                    {
                      Self::replace_global_import_references(
                        target_info,
                        &symbol_binding.symbol,
                        &local_name,
                      );
              ...
              ns_obj_exports.insert(used_name);
            }
          }
          for (export_name, symbol) in namespace_export_entries {
            if !ns_obj_exports.insert(export_name.clone()) {
              continue;
            }
            let symbol = Atom::from(symbol.as_str());
            let internal_name = namespace_internal_names
              .get(&symbol)
              .cloned()
              .unwrap_or(symbol);
            ns_obj.push(format!(
              "\n  {}: {}",
              property_name(&export_name).expect("should have property_name"),
              runtime_template.returning_function(&internal_name, "")
            ));
          }
...
"#,
  ...
  fn deconflict_symbols(
    ...
    all_used_names.extend(
      chunk_link
        ...
        .flat_map(|scope_info| scope_info.top_level_order.iter().cloned()),
    );

    // merge all all_used_names from scope-hoisted modules
    for id in chunk_link
      .hoisted_modules
      .iter()
      .chain(chunk_link.decl_modules.iter())
    {
      if let ModuleInfo::Concatenated(concate_info) = &concate_modules_map[id] {
    ...
    for id in &module_ids {
      ...
      if let ModuleInfo::Concatenated(concate_info) = info {
        let mut internal_names = concate_info.internal_names.clone();
        ...
        if let Some(import_map) = concate_info.import_map.take() {
          for ((source, attr), imported_atoms) in import_map {
            ...
            let mut ensure_import_stmt = false;

            if imported_atoms.namespace.is_none()
              && imported_atoms.namespace_aliases.is_empty()
              && imported_atoms.specifiers.is_empty()
              && imported_atoms.specifier_aliases.is_empty()
              && existing_namespace_import.is_none()
            {
              ensure_import_stmt = true;
            }

            for ns_import in imported_atoms
              .namespace
              .into_iter()
              .chain(imported_atoms.namespace_aliases)
            {
              if let Some(existing_local) = existing_namespace_import.as_ref() {
                if existing_local != &ns_import {
                  internal_names.insert(ns_import.clone(), ns_import.clone());
                  let import_spec = chunk_link
                    .raw_import_stmts
                    .entry(raw_import_source.clone())
                    .or_default();
                  if import_spec.ns_import.as_ref() != Some(&ns_import)
                    && !import_spec.ns_imports.contains(&ns_import)
                  {
                    import_spec.ns_imports.push(ns_import);
                  }
                }
              } else {
                let ns_local = concate_info
                  .internal_names
                  .get(&ns_import)
                  .cloned()
                  .unwrap_or(ns_import);
                all_used_names.insert(ns_local.clone());
                let import_spec = chunk_link
                  .raw_import_stmts
                  .entry(raw_import_source.clone())
                  .or_default();
                if import_spec.ns_import.is_none() {
                  import_spec.ns_import = Some(ns_local);
                } else if import_spec.ns_import.as_ref() != Some(&ns_local)
                  && !import_spec.ns_imports.contains(&ns_local)
                {
                  import_spec.ns_imports.push(ns_local);
                }
              }
            }

            if ensure_import_stmt {
              chunk_link
                .raw_import_stmts
                .entry(raw_import_source.clone())
                .or_default();
            ...
            let mut decl_before_module_locals = chunk_link
              .decl_before_modules
              .iter()
              .filter_map(|decl| {
                decl
                  ...
                  .map(|(local, _)| Atom::from(local))
              })
              .collect::<FxHashSet<_>>();

            for (imported_atom, atom, is_alias) in imported_atoms
              .specifiers
              .into_iter()
              .map(|atom| (atom.clone(), atom, false))
              .chain(
                imported_atoms
                  .specifier_aliases
                  .into_iter()
                  .map(|(imported_atom, atom)| (imported_atom, atom, true)),
              )
            {
              let is_referenced_in_source = concate_info
                .global_scope_ident
                .iter()
                .any(|ident| ident.id.sym == atom);
              ...
              if let Some(internal_atom) = total_imported_atoms
                .atoms
                .get(&imported_atom)
                .or_else(|| {
                  if imported_atom == "default"
                ...
                .cloned()
              {
                let mut local_atom = if is_alias {
                  concate_info
                    .internal_names
                    .get(&atom)
                    .cloned()
                  ...
                  concate_info
                    ...
                    .or_else(|| {
                      concate_info
                        .raw_export_map
                        .as_ref()
                        .and_then(|raw_export_map| raw_export_map.get(&atom))
                        .filter(|symbol| symbol.as_str() != "default")
                        .map(|symbol| Atom::from(symbol.as_str()))
                    })
                    .unwrap_or_else(|| internal_atom.clone())
                };
                if local_atom != internal_atom
                  && (Self::import_spec_uses_local(total_imported_atoms, &local_atom)
                    || decl_before_module_locals.contains(&local_atom))
                {
                  let renamed = Self::find_new_name_avoiding_current(&local_atom, &all_used_names);
                  all_used_names.insert(renamed.clone());
                  local_atom = renamed;
                }
                if local_atom != internal_atom {
                  if imported_atom == "default" {
                    chunk_link
                      .decl_before_modules
                      .insert(format!("const {local_atom} = {internal_atom};"));
                    decl_before_module_locals.insert(local_atom.clone());
                  } else {
                    total_imported_atoms
                      .named_imports
                      .push((imported_atom.clone(), local_atom.clone()));
                  }
                }
                if is_referenced_in_source && atom != local_atom {
                  Self::replace_global_import_references(concate_info, &atom, &local_atom);
                }
                internal_names.insert(atom.clone(), local_atom.clone());
                ...
                {
                  raw_export_map.insert(atom.clone(), local_atom.to_string());
                }
                continue;
              }

              let mut new_name = concate_info
                .internal_names
                .get(&atom)
                .cloned()
                .or_else(|| {
              ...
              if Self::import_spec_uses_local(total_imported_atoms, &new_name)
                || decl_before_module_locals.contains(&new_name)
              {
                let renamed = Self::find_new_name_avoiding_current(&new_name, &all_used_names);
                all_used_names.insert(renamed.clone());
                new_name = renamed;
              }
              ...
              if is_referenced_in_source && atom != new_name {
                Self::replace_global_import_references(concate_info, &atom, &new_name);
              }
              ...
              if imported_atom == "default" {
                ...
                total_imported_atoms
                  .atoms
                  .insert(imported_atom.clone(), new_name.clone());
              }
            }
          }
        }
        ...
        for (atom, ctxt) in concate_info.binding_to_ref.keys() {
          ...
          if internal_names.contains_key(atom) {
            continue;
          }
        ...
        let alias_updates = internal_names
          .iter()
          ...
          .collect::<Vec<_>>();
        for (key, resolved) in alias_updates {
          internal_names.insert(key, resolved);
        }
        ...
        if let Some(ref namespace_export_symbol) = concate_info.namespace_export_symbol
          ...
          && !concate_info
            .internal_names
            .contains_key(namespace_export_symbol)
        {
          let name: Atom =
        ...
        let namespace_object_name = {
          ...
          if let Some(pre_assigned) = pre_assigned {
            pre_assigned
          } else if let Some(namespace_object_name) = &concate_info.namespace_object_name {
            namespace_object_name.clone()
    ...
    for import_spec in chunk_link.raw_import_stmts.values() {
      ...
      for (_, atom) in &import_spec.named_imports {
        emitted_external_used_names.insert(atom.clone());
      }
            ...
            match info {
              ...
              rspack_core::ModuleInfo::Concatenated(mut concate_info) => {
                ...
                hooks
                  ...
                  .await?;
                let codegen_scope = codegen_res
                  .concatenation_scope
                  .as_ref()
                  .expect("should have concatenation scope");
                *concate_info = codegen_scope.current_module.clone();
                for (name, new_name) in &codegen_scope.top_level_renames {
                  concate_info
                    .internal_names
  ...
  */
      ...
      for name in entry_exports {
        ...
        match binding {
          Ref::Symbol(symbol_binding) => {
            ...
            match &mut concate_modules_map[&symbol_binding.module] {
              ModuleInfo::External(_) => {
              ...
              ModuleInfo::Concatenated(_) => {
                let mut symbol_binding = symbol_binding;
                let local_name = if symbol_binding.ids.is_empty() {
                  let module_graph = compilation.get_module_graph();
                  let local_name = if let Some(ext) = module_graph
                    .module_by_identifier(&symbol_binding.module)
                    .and_then(|m| m.as_external_module())
                    && (ext.get_external_type().as_str().starts_with("module")
                      || ext.resolve_external_type() == "module")
                    ...
                    let chunk_link = link.get_mut_unwrap(&ref_chunk);
                    Self::ensure_raw_import_binding(
                      chunk_link,
                      raw_import_source,
                      import_binding,
                      &symbol_binding.symbol,
                    )
                  } else if Self::ensure_existing_named_import_alias(
                    link.get_mut_unwrap(&ref_chunk),
                    &name,
                    &symbol_binding.symbol,
                  ) {
                    symbol_binding.symbol.clone()
                  ...
                  symbol_binding.symbol = local_name.clone();
          ...
          Ref::Inline(inlined_value) => {
            ...
            Self::reserve_concatenated_top_level_names(
              entry_module,
              module_graph,
              concate_modules_map,
  ...
  fn reserve_concatenated_top_level_names(
    module: ModuleIdentifier,
    module_graph: &ModuleGraph,
    concate_modules_map: &IdentifierIndexMap<ModuleInfo>,
    used_names: &mut FxHashSet<Atom>,
  ) {
    let Some(scope_info) = module_graph
      .module_by_identifier(&module)
      .and_then(|module| module.build_info().concatenated_module_scope_info.as_ref())
    else {
      return;
    };
    let Some(ModuleInfo::Concatenated(info)) = concate_modules_map.get(&module) else {
      return;
    };

    used_names.extend(scope_info.top_level_order.iter().map(|name| {
      info
        .internal_names
        .get(name)
        .cloned()
    ...
    for chunk_link in link.values_mut() {
      ...
      for (source, _) in &chunk_link.raw_star_exports {
        ...
        if let Some(import_spec) = chunk_link
          ...
          && import_spec.named_imports.is_empty()
      ...
      for (key, specifiers) in &chunk_link.raw_import_stmts {
        ...
        if specifiers.atoms.is_empty()
          && specifiers.named_imports.is_empty()
          && specifiers.default_import.is_none()
        {
          // import 'externals'
          continue;
        }
        ...
        if locals.iter().all(|(local, _)| {
          ...
          !chunk_link.used_names.contains(local)
            && !all_chunk_used_symbols.contains(local)
            && all_chunk_exported_symbols.contains_key(local)
        }) {
    ...
    if export_name.is_empty() {
      ...
      match exports_type {
        ...
        ExportsType::DefaultOnly => {
          ...
          if export_name.first().map(|item| item.as_str()) == Some("default")
            && let ModuleInfo::Concatenated(info) = info
            && let Some(raw_export) = info
              .raw_export_map
              .as_ref()
              .and_then(|map| map.get(&Atom::from("default")))
          {
