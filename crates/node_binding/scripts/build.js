	...
	return new Promise((resolve, reject) => {
		...
		cp.on("exit", (code) => {
			if (code === CARGO_SAFELY_EXIT_CODE) {
				...
				writeFileSync(dts,
					patchRealContentHashReferenceOptions(readFileSync(dts, "utf8"))
...
function patchRealContentHashReferenceOptions(content) {
	const replacement = `export type JsRealContentHashReferenceOptions = {
  asset: string
  ...
  kind: "filename"
	...
	return content.replace(
		`export interface JsRealContentHashReferenceOptions {
  asset: string
  referencedHash: string
  ownerHash?: string
  /** UTF-8 byte offsets in the final source. */
