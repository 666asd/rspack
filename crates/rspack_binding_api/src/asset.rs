...
impl From<AssetInfo> for rspack_core::AssetInfo {
  fn from(js_value: AssetInfo) -> Self {
    ...
    Self {
      ...
      css_unused_idents: css_unused_idents.map(|i| i.into_iter().map(Into::into).collect()),
      is_over_size_limit,
  ...
  fn from(value: rspack_core::AssetInfo) -> Self {
    ...
    Self {
      known: KnownAssetInfo {
        ...
        copied,
        javascript_module,
        css_unused_idents: css_unused_idents.map(|i| i.into_iter().map(String::from).collect()),
        is_over_size_limit,
