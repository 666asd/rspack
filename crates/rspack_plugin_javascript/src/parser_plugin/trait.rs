...
pub trait JavascriptParserPlugin {
  ...
  ///
  /// Stateless plugins may use `#[implemented_javascript_parser_hooks(stateless)]` to generate
  /// a shared `Arc` singleton accessor. Plugins that need custom construction may use
  /// `#[implemented_javascript_parser_hooks(shared = Plugin::new(...))]`.
...
mod tests {
  ...
  use super::{JavascriptParserPlugin, JavascriptParserPluginHooks};

  struct StatelessTestParserPlugin;

  #[rspack_macros::implemented_javascript_parser_hooks(stateless)]
  impl JavascriptParserPlugin for StatelessTestParserPlugin {}

  #[test]
  fn stateless_parser_plugin_macro_generates_shared_singleton() {
    let first = StatelessTestParserPlugin::shared();
    let second = StatelessTestParserPlugin::shared();

    assert!(Arc::ptr_eq(&first, &second));
    assert_eq!(
      first.implemented_hooks(),
      JavascriptParserPluginHooks::empty()
    );
  }
}
