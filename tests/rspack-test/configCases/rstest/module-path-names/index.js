...
it("Insert module path names with concatenateModules", () => {
	...
	expect(content).toContain(`const foo__filename = ${JSON.stringify(path.resolve(sourceDir, "./foo.js"))}`);
	expect(content).toContain(`const foo__dirname = ${JSON.stringify(path.resolve(sourceDir, "."))}`);
