...
确保开启了 [`output.module`](/config/output#outputmodule)。
...
将 externals 的默认类型指定为 `'modern-module'`。当 `output.library.type` 为 `'modern-module'` 时，这是默认值。
...
对于 CommonJS `require()`，Rspack 会保留 require 形式的 external。当 target 为 Node.js 时，生成的 ESM 产物会使用 [`'node-commonjs'`](#externalstypenode-commonjs)，通过 `createRequire` 构造 `require` 函数；其他 target 会使用 [`'commonjs'`](#externalstypecommonjs)，生成 `require()`。

对于类 Web target，请确保运行时存在可用的 `require` 函数；否则应避免使用 `require()` 形式的 external，改用 ESM import 形式。
