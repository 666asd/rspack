---
description: 'Rsdoctor 是一个针对 Rspack 的构建分析工具，可以直观地展示构建过程，例如编译时间、编译前后的代码变化、模块引用关系、重复模块等。'
---
...
Rspack 在内部使用 Tokio 线程池处理编译并行任务和异步任务。默认情况下，Tokio 会根据可用的 CPU 核心数自动确定工作线程的数量。

如果在构建过程中消耗了过多的 CPU 资源，可以通过设置 `TOKIO_WORKER_THREADS` 环境变量来限制工作线程的数量，例如：

```bash
# 需要安装 cross-env 包来设置环境变量
cross-env TOKIO_WORKER_THREADS=4 rspack build
```
