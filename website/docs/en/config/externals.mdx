...
- [`'modern-module'`](#externalstypemodern-module)
...
Specify the default type of externals as `'modern-module'`. This is the default when `output.library.type` is `'modern-module'`.
...
For CommonJS `require()`, Rspack keeps the external on the require path. When targeting Node.js, the generated ESM output uses [`'node-commonjs'`](#externalstypenode-commonjs) and creates a `require` function with `createRequire`. For other targets, it uses [`'commonjs'`](#externalstypecommonjs) and emits `require()`.

For web-like targets, make sure a `require` function is available at runtime, or avoid `require()`-style externals and use ESM import forms instead.
